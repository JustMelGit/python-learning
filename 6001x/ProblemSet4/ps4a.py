...
def updateHand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    updatedDic = hand.copy()
    for l in word:
        updatedDic[l] = updatedDic.get(l, 0) - 1
    return updatedDic
...
def isValidWord(word, hand, wordList):
    ...
    """
    # updatedHand = [k for k in hand.keys() if hand[k]>0]
    for l in word:
        x = word.count(l)
        try:
            if hand[l] < x:
...
print(isValidWord('chayote',dic3,loadWords()))
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print('"'+ user_input + '"' + ' earned ' + str(getWordScore(user_input,n)) + '. Total: ' + str(totalScore) + ' points')
    ...
    while True:
        ...
        if user_input_1 == 'n':
            ...
            if not handList:
                ...
                playHand(handList[-1],wordList,HAND_SIZE)
        elif user_input_1 == 'e':
            return
        else:
