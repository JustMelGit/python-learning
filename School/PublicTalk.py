...
themesList = ['1. How Well Do You Know God?',
...
'121. AWorldwide Brotherhood Saved From Calamity',
...
def publicTalks(file):
	...
	for e in file:
		if re.findall(Pat, e):
			Talks.append(e)
	for e in range(0,len(Talks),2):
		Date.append(Talks[e].strip())
	for e in range(1,len(Talks),2):
		No.append(Talks[e].strip())
	for e in No:
		mydict[int(e)] = No.count(e)
	return mydict


def Lectures(themes, file,themesList):
	import re
	Del = publicTalks(file)
	Talks = {}
	count = 0
	for e in themes:
		count+=1
	...
	for keys in Del:
		value = Del[keys]
		Talks[keys].append(value)
	for e in Talks:
		if len(Talks[e])==2:
			print(Talks[e][0], ':', Talks[e][1])
		else: print(str(e)+'.',re.match('([\d]+\.{1}) (.+)',themesList[e-1]).group(2),':not given')

themes = open(r'C:\Users\Justice\Documents\Public Talks.txt', encoding='utf8')
file = open(r'C:\Users\Justice\Documents\Schedule_2021 c.txt', encoding='utf8')
k = Lectures(themes, file,themesList)
